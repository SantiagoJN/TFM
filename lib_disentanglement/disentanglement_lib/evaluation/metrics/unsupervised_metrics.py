...
def unsupervised_metrics(ground_truth_data,
  ...
  cov_mus = np.cov(mus_train) # Matriz 20x20 con las covarianzas entre dimensiones del latent space
  ...
  # print(f'#############################################{scores["gaussian_wasserstein_correlation_norm"]}')
  ...
  mutual_info_score = np.sum(mutual_info_matrix) / (num_codes**2 - num_codes) # Hace el sumatorio y lo normaliza
