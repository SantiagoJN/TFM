...
@gin.configurable(
    ...
    blacklist=["ground_truth_data", "representation_function", "random_state",
               "artifact_dir"])
def compute_zmax(ground_truth_data,
  ...
  """ SMALL MODIFICATION TO THE FactorVAE score to compute ZMAX score 
  ...
  # print(f'~~~~~~~~~~~~~~~~~~~~')
  # print(f'~  TRAINING ZMAX   ~')

  # print(f'Training votes: \n{training_votes}')

  # print(f'Classifier = {classifier}')
  # print(f'other_index = {other_index}')
  # print(f'Correct votes = {training_votes[classifier, other_index]}')

  # print(f'Training accuracy = {np.sum(training_votes[classifier, other_index])} / {np.sum(training_votes)} = {np.sum(training_votes[classifier, other_index]) * 1. / np.sum(training_votes)}')
...
def _compute_variances(ground_truth_data,
  ...
  # print(f'Shape about observations: {observations.shape}')
...
def _generate_training_sample(ground_truth_data, representation_function,
  ...
  # ! Here are the only 2 differences wrt. FactorVAE score:
  # Fix the selected factor across mini-batch.
  # ! 1. Fix all factors but the selected one
  for f in range(factors.shape[1]):
    if f != factor_index:
      factors[:, f] = factors[0, f]

  observations = ground_truth_data.sample_observations_from_factors(
      factors, random_state)

  # print(f'Selected factor index: {factor_index}')
  # print(f'Factors after sampling observations: \n{factors}')
  # print("---------")

  representations = representation_function(observations)
  local_variances = np.var(representations, axis=0, ddof=1)
  # ! 2. Take the argmax, since the only varying factor is the selected one
  argmax = np.argmax(local_variances[active_dims] /
                     global_variances[active_dims])
  return factor_index, argmax
...
def _generate_training_batch(ground_truth_data, representation_function,
  ...
  votes = np.zeros((ground_truth_data.num_factors, global_variances.shape[0]),
                   dtype=np.int64)
  # print(f'NUM FACTORS: {ground_truth_data.num_factors}')
  for p in range(num_points):
    factor_index, argmin = _generate_training_sample(ground_truth_data,
                                                     ...
                                                     global_variances,
                                                     active_dims)
    # print(f'Point {p}, with factor index {factor_index} voting for {argmin}')
