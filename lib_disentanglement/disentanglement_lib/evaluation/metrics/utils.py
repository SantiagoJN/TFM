...
def generate_batch_factor_code(ground_truth_data, representation_function,
  ...
  factors = None # ? Cuenta como unsupervised?
  i = 0
  while i < num_points:
...
def discrete_mutual_info(mus, ys, is_discrete=True):
  ...
  for j in range(num_factors):
    for i in range(num_codes):
      ...
      # Each component (i,j) in matrix m is the mutual info between the latent representation i with the observation j
      # if is_discrete:
