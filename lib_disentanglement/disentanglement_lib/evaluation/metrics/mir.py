...
"""Mutual Information Ratio implementation

Based on "Disentanglement with biological constraints"
...
@gin.configurable(
    ...
    blacklist=["ground_truth_data", "representation_function", "random_state",
               "artifact_dir"])
def compute_mir(ground_truth_data,
  ...
  return _compute_mir(mus_train, ys_train)


# Implementation based on the Appendix A.1 from the referenced paper
def _compute_mir(mus_train, ys_train):
  """Computes score based on both training and testing codes and factors."""
  score_dict = {}
  discretized_mus = utils.make_discretizer(mus_train)
  I_nf = utils.discrete_mutual_info(discretized_mus, ys_train)
  r_n = np.max(I_nf, axis=1) / np.sum(I_nf, axis=1)
  n_f = ys_train.shape[0]
  # print(f'Num factors: {n_f}')
  # Assume all neurons are active
  score_dict["discrete_mir"] = (np.mean(r_n) - (1/n_f)) / (1 - (1/n_f))
  return score_dict
