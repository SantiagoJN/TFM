...
def compute_mig(ground_truth_data,
  ...
  # print(f'mus_train with shape {mus_train.shape}: \n{mus_train}')  # representations: Codes (num_codes, num_points)-np array
  # print(f'ys_train with shape {ys_train.shape}: \n{ys_train}')    # factors: Factors generating the codes (num_factors, num_points)-np array
...
def _compute_mig(mus_train, ys_train):
  ...
  # ! In our rendered datasets, information is not discrete but CONTINUOUS
  # m = utils.continuous_mutual_info()
  # print(f'\n\nMutual info:\n{m}')
