...
def compute_factor_vae(ground_truth_data,
  ...
  print(f'Classifier = {classifier}')
  print(f'other_index = {other_index}')
  print(f'Correct votes = {eval_votes[classifier, other_index]}')

  print(f'Eval accuracy = {np.sum(eval_votes[classifier, other_index])} / {np.sum(eval_votes)} = {np.sum(eval_votes[classifier, other_index]) * 1. / np.sum(eval_votes)}')
...
def _generate_training_sample(ground_truth_data, representation_function,
  ...
  # print(f'Observations of shape {observations.shape}')
  # print("-------")
  # for image in range(5):
  #       plt.imshow(observations.squeeze()[image]/255.0)
  #       plt.show()
  #       exit()
  ...
  # print(f'Local variances: {local_variances}\n----------------')
